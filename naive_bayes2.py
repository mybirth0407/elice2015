...
def naive_bayes(training1_sentence, training2_sentence, testing_sentence, alpha, prob1, prob2):
    ...
    C1 = create_BOW(training1_sentence)
    C2 = create_BOW(training2_sentence)
    x = create_BOW(testing_sentence)

    classify1 = calculate_doc_prob(C1, x, alpha) + math.log(prob1)
    classify2 = calculate_doc_prob(C2, x, alpha) + math.log(prob2)
