...
def run_PCA(df, num_components):
    # 4
    pca = sklearn.decomposition.PCA (num_components = 1)
    pca.fit (df)
...
def run_kmeans(pca_array, num_clusters, initial_centroid_indices):
    # 5
    initial_centroid_indices = np.array ([pca_array[i] for i in initial_centroid_indices])
    classifier = sklearn.cluster.KMeans (n_cluster = num_clusters, n_init = 1, init = initial_centroid_indices)
